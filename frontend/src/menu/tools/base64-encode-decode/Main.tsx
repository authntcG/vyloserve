...
function encodeTextToBase64(inputText: string, t: any): string {
    ...
    try {
        ...
        return t('tools.base64.encode_error');
    }
}

function decodeBase64Input(inputText: string, t: any): { output: string; preview: string | null } {
    if (!inputText) return { output: '', preview: null };
    try {
        let base64String = inputText.trim();
        const dataUriMatch = /^data:(.*?);base64,(.*)$/.exec(base64String);
        let preview: string | null = null;

        if (dataUriMatch) {
            const mimeType = dataUriMatch[1];
            base64String = dataUriMatch[2];
            preview = mimeType.startsWith('image/') ? inputText : null;
        }

        const binString = atob(base64String);
        const bytes = Uint8Array.from(binString, m => m.codePointAt(0) as number);
        return { output: new TextDecoder().decode(bytes), preview };
    } catch (e) { console.error(e);
        return { output: t('tools.base64.decode_error'), preview: null };
    }
}
...
interface EditorSectionProps {
    ...
    readonly inputType: 'text' | 'file';
    readonly inputText: string;
    readonly onInputTextChange: (v: string) => void;
    readonly outputText: string;
    readonly fileMeta: FileMeta | null;
    readonly fileInputRef: RefObject<HTMLInputElement | null>;
    readonly onFileUpload: (e: ChangeEvent<HTMLInputElement>) => void;
    readonly onCopy: () => void;
    readonly inputCardLabel: string;
    readonly t: any;
}
                ...
                <div className="w-full min-w-0 flex flex-col">
                    {mode === 'encode' && inputType === 'file' ? (
                        <div className="w-full h-36 border-2 border-dashed border-slate-300 dark:border-slate-700 rounded-lg flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-900/50 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors cursor-pointer relative overflow-hidden">
                            <input type="file" ref={fileInputRef} onChange={onFileUpload} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
...
interface PayloadInfoCardProps {
    readonly mode: 'encode' | 'decode';
    readonly inputType: 'text' | 'file';
    readonly fileMeta: FileMeta | null;
    readonly rawSizeLabel: string;
    readonly base64SizeLabel: string;
    readonly previewImage: string | null;
    readonly outputText: string;
    readonly t: any;
}

function PayloadInfoCard({ mode, inputType, fileMeta, rawSizeLabel, base64SizeLabel, previewImage, outputText, t }: PayloadInfoCardProps) {
    return (
...
export default function Base64Main() {
    ...
    useEffect(() => {
        if (mode === 'encode') {
            ...
            if (inputType === 'text') {
                setOutputText(encodeTextToBase64(inputText, t));
            } else {
        ...
        const { output, preview } = decodeBase64Input(inputText, t);
    ...
    const rawSizeLabel = (() => {
        if (mode !== 'encode') return formatBytes(new Blob([outputText]).size);
        if (inputType === 'file') return formatBytes(fileMeta?.size || 0);
        return formatBytes(new Blob([inputText]).size);
    })();

    const base64SizeLabel = mode === 'encode' ? formatBytes(new Blob([outputText]).size) : formatBytes(new Blob([inputText]).size);
