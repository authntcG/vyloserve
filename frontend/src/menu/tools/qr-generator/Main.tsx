...
export default function QrMain() {
    ...
    const handleDownload = () => {
        if (qrCode.current) {
            ...
            showToast(`QR Code berhasil diunduh sebagai ${downloadExt.toUpperCase()}!`, "success");
        }
    };
    ...
    return (
        <div className="flex flex-col w-full min-w-0">
            <PageHeader
                ...
                subtitle={<><span className="material-symbols-outlined text-[14px]">info</span> Buat kode QR dinamis dengan dukungan logo, pola matriks, dan multilink.</>}
            />
            ...
            <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 pb-8 mt-6 w-full min-w-0">
                ...
                <div className="col-span-1 xl:col-span-7 flex flex-col gap-6 w-full min-w-0">
                    ...
                    {/* Card 1: Data Konten */}
                    <Card title="Data QR Code" status="Input" gridCols="grid-cols-1">
                        <div className="w-full min-w-0 flex flex-col gap-4">
                            <div className="flex flex-col gap-1">
                                <label className="text-xs font-medium text-slate-500 uppercase">Tipe Konten</label>
                                <select value={contentType} onChange={(e) => setContentType(e.target.value as any)} className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white text-sm rounded-lg p-2.5 outline-none cursor-pointer">
                                    ...
                                    <option value="wifi">Koneksi Wi-Fi</option>
                                </select>
                            </div>
                            ...
                            {contentType === 'url' && (
                                <div className="flex flex-col gap-1">
                                    <label className="text-xs font-medium text-slate-500 uppercase">Masukkan Tautan URL</label>
                            ...
                            {contentType === 'text' && (
                                <div className="flex flex-col gap-1">
                                    <label className="text-xs font-medium text-slate-500 uppercase">Masukkan Teks Bebas</label>
                            ...
                            {contentType === 'email' && (
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div className="flex flex-col gap-1 md:col-span-2">
                                        <label className="text-xs font-medium text-slate-500 uppercase">Alamat Tujuan</label>
                                    ...
                                    <div className="flex flex-col gap-1 md:col-span-2">
                                        <label className="text-xs font-medium text-slate-500 uppercase">Isi Pesan</label>
                            ...
                            {contentType === 'wifi' && (
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div className="flex flex-col gap-1 md:col-span-2">
                                        <label className="text-xs font-medium text-slate-500 uppercase">Nama Jaringan (SSID)</label>
                                    ...
                                    <div className="flex flex-col gap-1">
                                        <label className="text-xs font-medium text-slate-500 uppercase">Keamanan</label>
                            ...
                            <div className="mt-2 p-3 bg-slate-100 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-lg">
                                <span className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Data Mentah Output</span>
                    ...
                    <Card title="Desain Visual" status="Design" gridCols="grid-cols-1 md:grid-cols-2">
                        <div className="flex flex-col gap-2 w-full min-w-0">
                            <label className="text-xs font-medium text-slate-500 uppercase">Pola Titik (Dots)</label>
                        ...
                        <div className="flex flex-col gap-2 w-full min-w-0">
                            <label className="text-xs font-medium text-slate-500 uppercase">Bentuk Sudut (Corners)</label>
                            <div className="flex items-center gap-2">
                                <select value={cornerType} onChange={(e) => setCornerType(e.target.value as CornerSquareType)} className="flex-1 bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white text-sm rounded-lg p-2.5 outline-none cursor-pointer">
                                    <option value="square">Kotak</option>
                                    <option value="extra-rounded">Extra Bulat</option>
                                    <option value="dot">Titik Besar</option>
                                </select>
                    ...
                    {/* Card 3: Logo */}
                    <Card title="Sisipkan Logo" status="Optional" gridCols="grid-cols-1">
                        <div className="w-full min-w-0 flex flex-col gap-4">
                            ...
                            <div className="flex flex-col gap-2 w-full min-w-0">
                                <label className="flex justify-between text-xs font-medium text-slate-500 uppercase">
                                    <span>Skala Logo (Menutupi Area)</span>
                ...
                <div className="col-span-1 xl:col-span-5 w-full min-w-0">
                    <div className="sticky xl:top-6">
                        <Card title="Live Preview" status="Ready" gridCols="grid-cols-1">
