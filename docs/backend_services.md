...
### 11.1 Membaca Isi File Log Persisten (Apache `error_log`/`access_log`, Database `db_startup.log`/native `.err`)

Selain event log runtime lewat `emit_log()`/`vylo_log` (§3), aplikasi ini juga bisa menampilkan isi **file log yang tersimpan di disk** langsung di dalam UI (lihat `docs/known_bugs.md` #22–#25 untuk konteks fitur ini dan `docs/frontend_ui.md` untuk sisi frontend-nya):

- `core/utils/file_utils.read_log_tail(file_path, max_lines=300, ...)`: membaca **hanya N baris terakhir** sebuah file teks tanpa memuat seluruh isinya ke memori — aman dipakai untuk file yang tidak dibatasi ukurannya (Apache `logs/error_log`/`logs/access_log` tidak punya rotasi, MySQL menulis file `.err` sendiri juga tanpa rotasi). Melakukan retry singkat (default 3x, jeda 0.2s) kalau file sedang dikunci proses lain di Windows (`PermissionError`/`OSError`), lalu tetap melempar error aslinya kalau semua retry habis — TIDAK menelan error diam-diam (kontras dengan bug lama di `docs/known_bugs.md` #12).
- `ApacheManager.get_log_content(log_type)` (`'error'` atau `'access'`) → `Api.get_apache_log_content(log_type)`. Path diresolusi dari `get_status()['path']` (instalasi Apache yang sedang aktif), **bukan** dari path bebas kiriman frontend — mencegah path traversal.
- `DatabaseManager.get_log_content(db_id, log_type='startup')` (`'startup'` = `db_startup.log` milik VyloServe sendiri, tersedia utk semua engine; `'native'` = file `.err` native MySQL/MariaDB, dicari via `glob` di `dataDir` karena nama filenya mengikuti hostname mesin — tidak tersedia untuk PostgreSQL karena `logging_collector` default off) → `Api.get_database_log_content(db_id, log_type)`. Path juga diresolusi dari record `databases.json` by `db_id`, bukan path bebas dari frontend.
- Kedua method mengikuti kontrak response standar `{status, data?, message?, args?}` yang sama dengan API lain.

### 11.2 Log Watcher — Menyalurkan File Log ke System Logs Secara Otomatis

Selain dibaca manual lewat §11.1, isi file `error_log`/`access_log` (Apache) dan `db_startup.log`/`*.err` (Database) juga disalurkan **otomatis** sebagai entri baru di System Logs (event `vylo_log`) tanpa user perlu membuka modal apa pun — mekanismenya:

- `core/utils/file_utils.read_new_lines(file_path, offset)`: membaca baris **BARU** sejak `offset` byte terakhir (gaya `tail -f`), mengembalikan `(baris_baru, offset_baru)`. Reset ke offset 0 kalau file mengecil dari offset sebelumnya (mis. `db_startup.log` ditulis ulang mode `'w'` tiap start baru). Kalau file sedang terkunci proses lain, mengembalikan offset yang SAMA (bukan error) — aman karena dipanggil berkala, otomatis dicoba lagi di polling berikutnya.
- `ApacheManager.tail_new_logs()` / `DatabaseManager.tail_new_logs()`: masing-masing dipanggil secara berkala, hanya aktif untuk service yang **sedang berjalan**. Kunjungan PERTAMA ke sebuah file men-*baseline* offset ke ukuran file saat itu (gaya `tail -f`) — **tidak** me-replay seluruh histori lama sebagai log baru saat aplikasi baru dibuka. `error_log` selalu level `error`, `access_log` selalu level `info`; untuk database, level `error` kalau baris mengandung kata "error" (case-insensitive), selain itu `info` — heuristik sederhana, bukan parser log yang presisi.
- `Api.start_log_watcher(interval=2.0)`: menjalankan satu thread daemon (pola sama seperti tray icon di `main.py`) yang memanggil kedua `tail_new_logs()` di atas tiap `interval` detik, lalu meneruskan baris baru lewat `self.api.emit_log(line, level, {}, source_override='ApacheFileLog'|'DatabaseFileLog')`. Dipanggil sekali dari `main.py` setelah `api.set_window(window)` (emit_log butuh window untuk bisa mengirim event). `Api.stop_log_watcher()` menghentikannya dengan bersih, dipanggil dari `AppLifecycle.perform_exit()`.
- **`source_override`** (`Api.emit_log(message, level, args, source_override=None)`): dipakai untuk memaksa label kategori tertentu, mem-bypass `_resolve_event_source()` (§3). Diperlukan karena baris file log (`tail_new_logs()`) dan pesan sistem biasa (`start_server()`, dst.) sama-sama method di class `ApacheManager`/`DatabaseManager` yang sama — tanpa override, keduanya otomatis dapat `source` yang identik dan tidak bisa difilter terpisah di modal "System Logs". Dengan override, keduanya jadi kategori independen: `ApacheManager`/`DatabaseManager` (pesan sistem) vs `ApacheFileLog`/`DatabaseFileLog` (baris file log). Lihat `docs/known_bugs.md` #26.
- **Kenapa thread polling, bukan file-watcher OS-native (`watchdog`, dsb.)**: proyek ini belum punya dependency semacam itu, dan pola thread-daemon-dengan-interval sudah dipakai di tempat lain (`main.py` tray icon) — menambah dependency baru untuk kebutuhan sederhana ini dianggap tidak sepadan. Trade-off: keterlambatan deteksi baris baru maksimum `interval` detik (default 2 detik), dianggap dapat diterima untuk fitur tampilan log, bukan jalur data kritikal.
